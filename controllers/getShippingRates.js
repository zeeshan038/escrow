...
module.exports.getCanadaPostRates = async (req, res) => {
    console.log("Received request to fetch Canada Post rates");
    ...
    try {
        ...
        const jsonData = {
            "mailing-scenario": {
                ...
                "origin-postal-code": "H2B1A0",
                "destination": {
                    "domestic": { "postal-code": "K1K4T3" }
                }
            }
        };
...
module.exports.getUSPSRates = async (req, res) => {
    ...
    try {
        const { originZip, destinationZip, pounds, ounces, length, width, height } = req.body;

        // Validate required fields
        if (!originZip || !destinationZip || !pounds || !ounces || !length || !width || !height) {
        ...
        // Make API request
        const response = await axios.get(`${USPS_API_URL}&XML=${encodeURIComponent(rateRequestXML)}`);

        // Convert XML to JSON
        xml2js.parseString(response.data, { explicitArray: false }, (err, result) => {
            if (err) {
                console.error("Error parsing XML:", err);
                return res.status(500).json({ error: "Failed to parse USPS response" });
            }
